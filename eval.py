    ...
    for subject in subjects:
        ...
        if method == "NeuralUDF":
            ...
            img_dir = os.path.join(root_dir, 
                                f"exp/udf/garment_sphere/{subject}/udf_garment_woblending_mixsample_ft_mask/novel_view")
            pbar = tqdm(glob.glob(os.path.join(img_dir, 'pred*.png')))

        elif method == "NeUDF":
            res=256
            img_dir = os.path.join(root_dir, 
                               f"exp/{subject}/wmask_open/validations_fine")
            pbar = tqdm(glob.glob(os.path.join(img_dir, '00400000_0*.png')))
            
        elif method == "NeAT":
            res=1024
            img_dir = os.path.join(root_dir, 
                               f"exp/{subject}/wmask/validations_fine")
